...
-- ----------------------------------------------------------------------------
-- 12. PLANIFICACIÓN: cuentas, metas y movimientos recurrentes
-- Esta misma sección existe también en supabase-planning-update.sql para
-- actualizar instalaciones ya creadas sin volver a ejecutar todo el esquema.
-- ----------------------------------------------------------------------------
create table if not exists public.financial_accounts (
  ...
  reconciled_balance numeric(12,2), reconciled_at date,
...
create table if not exists public.financial_goals (
  ...
  group_id uuid not null references public.finance_groups(id) on delete cascade,
  created_by uuid not null references public.profiles(id),
  title text not null, target_amount numeric(12,2) not null check (target_amount > 0),
  target_date date, initial_amount numeric(12,2) not null default 0 check (initial_amount >= 0),
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table if not exists public.goal_contributions (
  ...
  group_id uuid not null references public.finance_groups(id) on delete cascade,
  created_by uuid not null references public.profiles(id), amount numeric(12,2) not null check (amount > 0),
  fecha date not null default current_date, note text, created_at timestamptz not null default now()
);
create table if not exists public.recurring_transactions (
  id uuid primary key default gen_random_uuid(), group_id uuid not null references public.finance_groups(id) on delete cascade,
  created_by uuid not null references public.profiles(id), kind text not null check (kind in ('income','expense')),
  category text not null, subcategory text, description text, amount numeric(12,2) not null check (amount > 0), method text,
  day_of_month smallint not null check (day_of_month between 1 and 31), start_date date not null default current_date,
  end_date date, active boolean not null default true, account_id uuid references public.financial_accounts(id) on delete set null,
...
create trigger trg_financial_goals_updated before update on public.financial_goals for each row execute function public.set_updated_at();
drop trigger if exists trg_recurring_transactions_updated on public.recurring_transactions;
create trigger trg_recurring_transactions_updated before update on public.recurring_transactions for each row execute function public.set_updated_at();
...
begin
  if new.account_id is not null then
    ...
    if reference_group is null or reference_group <> new.group_id then raise exception 'La cuenta debe pertenecer al mismo grupo financiero.'; end if;
  end if;
  if tg_table_name in ('expenses','incomes') and new.recurring_id is not null then
    ...
    if reference_group is null or reference_group <> new.group_id then raise exception 'La recurrencia debe pertenecer al mismo grupo financiero.'; end if;
  end if;
  return new;
end; $$;
...
begin
  ...
  if goal_group is null or goal_group <> new.group_id then raise exception 'La meta debe pertenecer al mismo grupo financiero.'; end if;
  return new;
end; $$;
drop trigger if exists trg_expenses_planning_refs on public.expenses;
create trigger trg_expenses_planning_refs before insert or update on public.expenses for each row execute function public.validate_planning_references();
drop trigger if exists trg_incomes_planning_refs on public.incomes;
create trigger trg_incomes_planning_refs before insert or update on public.incomes for each row execute function public.validate_planning_references();
drop trigger if exists trg_debt_payments_planning_refs on public.debt_payments;
create trigger trg_debt_payments_planning_refs before insert or update on public.debt_payments for each row execute function public.validate_planning_references();
drop trigger if exists trg_recurring_planning_refs on public.recurring_transactions;
create trigger trg_recurring_planning_refs before insert or update on public.recurring_transactions for each row execute function public.validate_planning_references();
drop trigger if exists trg_goal_contribution_group on public.goal_contributions;
create trigger trg_goal_contribution_group before insert or update on public.goal_contributions for each row execute function public.validate_goal_contribution_group();
...
declare t text;
begin
  foreach t in array array['financial_accounts','financial_goals','goal_contributions','recurring_transactions'] loop
    execute format('drop policy if exists "members select %1$s" on public.%1$s', t);
    execute format('create policy "members select %1$s" on public.%1$s for select using (public.is_group_owner(group_id) or public.is_shared_viewer(group_id))', t);
    execute format('drop policy if exists "owner inserts %1$s" on public.%1$s', t);
    execute format('create policy "owner inserts %1$s" on public.%1$s for insert with check (public.is_group_owner(group_id) and created_by = auth.uid())', t);
    execute format('drop policy if exists "owner updates %1$s" on public.%1$s', t);
    execute format('create policy "owner updates %1$s" on public.%1$s for update using (public.is_group_owner(group_id))', t);
    execute format('drop policy if exists "owner deletes %1$s" on public.%1$s', t);
    execute format('create policy "owner deletes %1$s" on public.%1$s for delete using (public.is_group_owner(group_id))', t);
  end loop;
end $$;
